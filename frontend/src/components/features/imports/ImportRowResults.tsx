...
import { IMPORT_ROW_STATUS_COLORS, IMPORT_ROW_STATUS_LABELS } from "@/types/imports"
  ...
  return (
    <Card>
      ...
      {data && data.items.length === 0 && (
        <p className="px-6 pb-6 text-sm text-slate-500">No row results yet.</p>
      )}
            ...
            <TableBody>
              {data.items.map((row) => (
                <TableRow key={row.id}>
                  ...
                  <TableCell>
                    <Badge className={IMPORT_ROW_STATUS_COLORS[row.status]}>
                  ...
                  <TableCell>
                    {row.error_message ? (
                      <div>
                        <span className="text-xs font-medium text-red-600">{row.error_code}</span>
                        <p className="text-xs text-slate-500">{row.error_message}</p>
                      </div>
                    ) : (
